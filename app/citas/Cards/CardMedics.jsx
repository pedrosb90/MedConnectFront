  ...
  const paginatedMedicos = medicos.slice(
    ...
    return(
      <div className={styles.cards + ' flex  justify-center'}>
          ...
          >
            <svg className="h-8 w-8 text-white"  width="24" height="24" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">  <path stroke="none" d="M0 0h24v24H0z"/>  <polyline points="15 6 9 12 15 18" /></svg>
            Volver
          
          </button>
        {medicos.length ? paginatedMedicos.map(med=>{
      ...
      <div className="flex flex-col items-center pb-4">
        <img
          ...
          alt="Bonnie image"
        />
         <Link href={`/medicos/${med.id}`}><h5 className={`mb-1 text-xs font-medium text-gray-900 dark:text-white text-center ${styles.name}`}>{med.first_name ?'Dr. ' +med.first_name:'...Loading'}</h5></Link>
        <span className="text-xs text-gray-500 dark:text-gray-400">{med.last_name ? med.last_name:'..Loading'}</span>
       ...
       <button
          ...
          >
            Next
            <svg className="h-8 w-8 text-white"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeLinecap="round"  strokeLinejoin="round">  <polyline points="9 18 15 12 9 6" /></svg>
          </button>
      </div>
