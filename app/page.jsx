...
export default function Home() {
  ...
  const handlerPrev = () =>{
    ...
    const nextIndex = isFirstSlide ? especialidades.length -1 : currentIndex -1;
    ...
    <main>
      <div className="flex flex-col gap-10">
        ...
        <div className="max-w-[800px] h-[500px] w-full m-auto py-16 px-4 relative group">
          <div style={{backgroundImage: `url(${especialidades[currentIndex].url})`}} className="flex justify-between items-end w-full h-full rounded-2xl bg-center bg-cover duration-500 text-4xl font-bold ">  {especialidades[currentIndex].name} <Link href={`/Especialidades`} className="font-normal rounded-lg bg-cimPallete-300 opacity-60 text-2xl cursor-pointer"><h1>Ver Mas Especialidades</h1></Link></div>
          {/* Left Arrow */}
          ...
          <div className="flex top-4 justify-center py-2">
            ...
            {especialidades.map(( _, index)=>(
              <div key={index} onClick={()=> goToSlide(index) } className="cursor-pointer text-4xl">
